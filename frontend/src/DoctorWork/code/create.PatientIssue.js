...
export default class CreatePatientIssue extends Component{
  state = {
    index: 0,
    direction: null,
    isError: false,
    ...
    await web3.eth.getAccounts(function(error, accounts) {
      ...
      MetaCoinContract.deployed().then(function(instance){
        proposalInstance = instance;
          return proposalInstance.submitIssue(title, startDate, endDate, occurrence, severity, referredBy, isAllergy, patientAddress, accounts[0], {
            from: accounts[0]
          });
          ...
          <div className='row ro dev'>
            <div className='col-md-6 col-sm-12 dev'>
              <div className='bc dev'>
                <div className='intro dev'>
                  <h1 id='name'>CREATE PATIENT ISSUE</h1>
                  <span id='quoto'>
            ...
            <Form className="reserveform">
              ...
              <Form.Group controlId="formBasicPassword">
                ...
                <Form.Control type="text" defaultValue={this.state.patientAddress} placeholder="doctor address" onChange={(event)=> {
                  this.setState({patientAddress: event.target.value,isError:false});
                }} />
              </Form.Group>
